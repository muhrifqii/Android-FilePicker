...
public class PhotoDirLoaderCallbacks
  ...
  public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
    ...
    while (data.moveToNext()) {
      ...
      boolean b1 = !path.toLowerCase().endsWith("gif") && !path.toLowerCase()
          .endsWith("mp4");
      if (b1) {
        if (!directories.contains(photoDirectory)) {
          if(addPhoto(fileSize, photoDirectory, imageId, fileName, path,
              mediaType)) {
  ...
  private boolean addPhoto(String fileSize,
    ...
    if (fileSize != null) {
      if (Double.parseDouble(fileSize) <= PickerManager.getInstance()
          .getMaxFileSize() && Double.parseDouble(fileSize) > 0) {
